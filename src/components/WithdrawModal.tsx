import {  createWithdrawCoinIx, createWithdrawPcIx } from "@/solana/instructions";
...
const WithdrawModal = ({ isOpen, closeModal, tokenType,onOpenChange }: Props) => {
  ...
  const handleWithdraw = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      setProcessing(true);
      if (tokenType === "pc") {
        const signature = await createWithdrawPcIx({
          amount: quantity,
          authority: connectedWallet?.publicKey,
          coinMint: new PublicKey(selectedMarket?.pcMint),
        ...
        setTxHash(signature ?? "");
        toast.success("Withdraw txn sent ");
        await fetchOpenOrders();
      }
      if (tokenType === "coin") {
        const signature = await createWithdrawCoinIx({
          amount: quantity,
          authority: connectedWallet?.publicKey,
          coinMint: new PublicKey(selectedMarket?.pcMint),
          pcMint: new PublicKey(selectedMarket?.pcMint),
      ...
      console.log("Error in WithdrawModal/handleWithdraw", err);
    } finally {
